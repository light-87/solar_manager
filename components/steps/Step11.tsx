...
import { Customer, Step11Data } from '@/types';

interface Step11Props {
  customer: Customer;
  stepData: Step11Data | null;
  onSave: (data: Step11Data) => Promise<void>;
}

export default function Step11({ customer, stepData, onSave }: Step11Props) {
  const [formData, setFormData] = useState<Step11Data>({
    mseb_inspection: stepData?.mseb_inspection || 'no',
      ...
      <div>
        ...
        <div className="flex gap-4">
          <label className="flex items-center gap-2">
            <input
              type="radio"
              name="mseb_inspection"
              value="done"
              checked={formData.mseb_inspection === 'done'}
              onChange={(e) => setFormData({ ...formData, mseb_inspection: e.target.value as 'done' | 'no' })}
          ...
          <label className="flex items-center gap-2">
            <input
              ...
              name="mseb_inspection"
              value="no"
              checked={formData.mseb_inspection === 'no'}
              onChange={(e) => setFormData({ ...formData, mseb_inspection: e.target.value as 'done' | 'no' })}
              className="w-4 h-4 text-amber-600 focus:ring-amber-600"
            />
