...
import { Customer, Step14Data } from '@/types';

interface Step14Props {
  customer: Customer;
  stepData: Step14Data | null;
  onSave: (data: Step14Data) => Promise<void>;
}

export default function Step14({ customer, stepData, onSave }: Step14Props) {
  const getTodayDate = () => {
    const today = new Date();
    return today.toISOString().split('T')[0];
  };

  const [formData, setFormData] = useState<Step14Data>({
    mail_sent: stepData?.mail_sent || 'no',
        ...
        <div className="flex gap-4">
          <label className="flex items-center gap-2">
            <input
              ...
              name="mail_sent"
              value="done"
              checked={formData.mail_sent === 'done'}
              onChange={(e) => setFormData({ ...formData, mail_sent: e.target.value as 'done' | 'no' })}
